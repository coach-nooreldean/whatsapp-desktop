...
const format = values => {
  return [
    ...
    `stealth = ${!!values['privacy.stealth']}`,
    `auto-blur = ${values['privacy.auto-blur'] !== false}`,
    `hover-reveal = ${values['privacy.hover-reveal'] !== false}`,
    `blur-contacts = ${values['privacy.blur-contacts'] !== false}`,
    '',
    '[lock]',
    '# Passcode protection and auto-lock behavior.',
    `enabled = ${!!values['lock.enabled']}`,
    `timeout = ${values['lock.timeout'] != null ? values['lock.timeout'] : 15}`,
    `auto-lock-on-system-lock = ${values['lock.auto-lock-on-system-lock'] !== false}`,
    '',
    '[accounts]',
    '# Minutes of inactivity before background account sleeps.',
    `hibernation-minutes = ${values['accounts.hibernation-minutes'] != null ? values['accounts.hibernation-minutes'] : 30}`,
    '',
    '[mpris]',
    `enabled = ${values['mpris.enabled'] !== false}`,
    '',
    '[fonts]',
